obj/Equipment/weapon
	AdvancedWeighted_Swords
		name = "Advanced Weighted Sword"
		density = 0
		equiped = 0
		value = 500
		icon = 'WeightedSword.dmi'
		var/level=5
		var
			wsword
		Click()
			if(src in usr:contents)
				if(!usr.swordd_eq)
					if(usr.weapon == "EMPTY")
						if(!usr.swordd)
							strboost = (usr.strength/15)
							usr.overlays += wsword
							usr<< "You have equiped the: <u>[src]</u>."
							suffix = "(Worn)"
							usr.weapon = src.name
							usr.strength += strboost
							usr.swordd = 1
							usr.swordd_eq = 1
							usr.weightsword = level
							src.equiped = 1
					else
		...
		verb
			...
			Drop()
				...
				if(!src.equiped)
					...
					usr.inven_min -= 1
				else usr<< "You're unable to drop: <u>[src]</u>."
	Weighted_Swords
		name = "Weighted Sword"
		density = 0
		equiped = 0
