...
obj/Techniques/Bio_Android/Absorb
	verb/Absorb(mob/M in get_step(usr,usr.dir))
		...
		if(usr.doing)
			...
			return
		if(M.race == "Android")
			usr<< "<font color=blue>Error: </font>[M] is completely mechanical, you cannot absorb them!"
			return
		...
		if(M.tourny||usr.tourny)
			usr<<"You cannot absorb in a tournament"
			return
		...
		if(!M.KO && M.powerlevel > 0)
			...
			if(prob(80))
				...
				if(M.key)
					...
					if(M.powerlevel_max/(42+usr.absorb_kill) > usr.powerlevel_max) {usr<<"<font color=aqua>You have gained [usr.powerlevel_max] powerlevel.";usr.powerlevel_max += usr.powerlevel_max}
					else {usr<<"<font color=aqua>You have gained [M.powerlevel_max/(52+usr.absorb_kill)] powerlevel.";usr.powerlevel_max += round(M.powerlevel_max/(42+usr.absorb_kill))}
					if(M.ki_max/(42+usr.absorb_kill) > usr.ki_max) {usr<<"<font color=aqua>You have gained [usr.ki_max] ki.";usr.ki_max += usr.ki_max}
					else {usr<<"<font color=aqua>You have gained [M.ki_max/(52+usr.absorb_kill)] ki.";usr.ki_max += round(M.ki_max/(42+usr.absorb_kill))}
					if(M.strength_max/(42+usr.absorb_kill) > usr.strength_max) {usr<<"<font color=aqua>You have gained [usr.strength_max] strength.";usr.strength_max += usr.strength_max}
					else {usr<<"<font color=aqua>You have gained [M.strength_max/(52+usr.absorb_kill)] strength.";usr.strength_max += round(M.strength_max/(42+usr.absorb_kill))}
					if(M.defence_max/(42+usr.absorb_kill) > usr.defence_max) {usr<<"<font color=aqua>You have gained [usr.defence_max] defence.";usr.defence_max += usr.defence_max}
					else {usr<<"<font color=aqua>You have gained [M.defence_max/(52+usr.absorb_kill)] defence.";usr.defence_max += round(M.defence_max/(42+usr.absorb_kill))}
