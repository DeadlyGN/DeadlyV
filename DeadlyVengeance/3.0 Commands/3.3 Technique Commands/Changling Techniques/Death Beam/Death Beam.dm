...
mob
	proc
		DBeam_Shoot()
			var/obj/tech/H = new/obj/tech/death_beam(o=src)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Death Beam was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Death Beam, but it was too strong!"
...
obj/tech/death_beam
	...
	Move()
		var/O = new/obj/trail/DBtrail(src.loc)
		O:icon_state = "trail"
		O:dir = src.dir
		return ..()
obj/trail/DBtrail
