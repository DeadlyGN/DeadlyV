...
mob
	proc
		Hyper_Tornado_Shoot()
			var/obj/tech/H = new/obj/tech/hyper_tornado(o=src)
			H.ki_damage = round(src.powerlevel_max * 14)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Hyper Tornado was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Hyper Tornado, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/hyper_tornado
