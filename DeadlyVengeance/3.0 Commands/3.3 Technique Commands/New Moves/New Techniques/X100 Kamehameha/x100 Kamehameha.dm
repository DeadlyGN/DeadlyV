...
mob
	proc
		x100_Kame_Shoot()
			var/obj/tech/H = new/obj/tech/x100_kame(o=src)
			H.ki_damage = round(src.powerlevel_max * 1)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s 100x Kamehameha slams into you"
								src<<"\red Your 100x Kamehameha slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your 100x Kamehameha was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s 100x Kamehameha, but it was too strong!"
