...
mob
	proc
		Acid_Shoot()
			var/obj/tech/H = new/obj/tech/acid(o=src)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					sleep(3)
					view(6) << "<font color = white>[src]:</font> BURN!!"
					src.Icon_State("")
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Acid Burn was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Acid Burn, but it was too strong!"
