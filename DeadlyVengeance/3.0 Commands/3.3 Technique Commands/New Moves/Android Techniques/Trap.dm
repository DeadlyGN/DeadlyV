	...
	verb
		Trap_Shooter()
			...
			for(var/area/house/S in view(4,src))
				...
				return
			for(var/turf/Planet_Gravitys/Supreme_Kai/A in view(6,src))
				src<< "They're in a Safe Zone!"
				return
			for(var/turf/Planet_Gravitys/King_Kai/D in view(6,src))
				src<< "They're in a Safe Zone!"
				return
			Trap_Shooter_Shoot()
			src.afk_time = 0
mob
	proc
		Trap_Shooter_Shoot()
			var/obj/tech/H = new/obj/tech/TS(o=src)
			H.ki_damage = round(usr.powerlevel_max * 0.80)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6,src) << "<font color = white>[src]:</font> Trap Shooter!!!"
					sleep(13)
					view(6,src) << "<font color = white>[src]:</font> HAA!!!"
					src.overlays -= kamehameha_charge
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Trap Shooter was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Trap Shooter, but it was too strong!"
