...
mob
	proc
		Big_Bang_Kamehameha_X100_Shoot()
			var/obj/tech/H = new/obj/tech/Big_Bang_Kamehameha_X100_Blast(o=src)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Big Bang Kamehawave slams into you"
								src<<"\red Your Big Bang Kamehawave slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Big Bang Kamehawave was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Big Bang Kamehawave, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/Big_Bang_Kamehameha_X100_Blast
	icon = 'LG3.dmi'