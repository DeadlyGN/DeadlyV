...
mob
	proc
		Whirlwind_Spin_Shoot()
			var/obj/tech/H = new/obj/tech/www1(o=src)
			H.ki_damage = round(src.powerlevel_max * 0.85)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Whirlwind Spin was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Whirlwind Spin, but it was too strong!"
