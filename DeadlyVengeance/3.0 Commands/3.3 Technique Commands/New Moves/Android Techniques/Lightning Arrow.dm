	...
	verb
		Falcon_Punch(mob/M in oview(6))
			...
			for(var/turf/Planet_Gravitys/King_Kai/D in view(6,M))
				...
				return
			src.afk_time = 0
			var/ki_damage = round(((src.ki_skill / src.ki_skill_max) * src.level) + src.powerlevel_max * 48)
			var/ki_cost = round(src.ki_max * 0.82)
			if(src.ki >= ki_cost)
				if(!src.doing)
					src.ki_lock = 1
					...
					view(6) << "<font color = blue>[src]:<font color=yellow> Falcon"
					sleep(14)
					view(6) << "<font color = blue>[src]:<font color=yellow> PAWNCH!!!"
					src.overlays -= ray_charge
					if(!M||M.safe||M.dead||M.tourny)
						src.doing=0
						src.frozen=0
						return
					missile('Lightning Arrow.dmi', src, M)
					spawn(5) if(src.doing) src.doing --
					spawn(5) src.frozen = 0
					spawn(150) src.ray_lock = 0
					if(prob(7))
						M.powerlevel = 0
						M << "\red [src.name]'s Falcon Punch blows a hole through your Chest!"
					else
						M.Damage(round(ki_damage))
						M << "\yellow [src.name]'s Falcon Punch plows into you"
