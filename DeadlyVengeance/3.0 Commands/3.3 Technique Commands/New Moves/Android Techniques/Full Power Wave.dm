...
mob
	proc
		Full_Power_Energy_Wave_Shoot()
			var/obj/tech/H = new/obj/tech/FPEW(o=src)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Energy Wave slams into you"
								src<<"\red Your Energy Wave slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Energy Wave was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Energy Wave, but it was too strong!"
