...
mob
	proc
		Final_Galick_Cannon_Shoot()
			var/obj/tech/H = new/obj/tech/FC(o=src)
			H.ki_damage = round(src.powerlevel_max * 0.87)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Galick Cannon slams into you"
								src<<"\red Your Galick Cannon slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Galick Cannon was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Galick Cannon, but it was too strong!"
