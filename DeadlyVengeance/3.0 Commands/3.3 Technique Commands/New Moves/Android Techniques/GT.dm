...
mob
	proc
		Galatic_Tyrant_Shoot()
			var/obj/tech/H = new/obj/tech/GT(o=src)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Galatic Tyrant was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Galatic Tyrant, but it was too strong!"
