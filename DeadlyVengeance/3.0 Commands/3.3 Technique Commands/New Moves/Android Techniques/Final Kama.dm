...
mob
	proc
		Final_Kamehameha_Shoot()
			var/obj/tech/H = new/obj/tech/Final_Kamehameha_Blast(o=src)
			var/ki_cost = round(src.ki_max*0.35)
			H.ki_damage = round((src.powerlevel*1.45)+(src.ki*0.85))
			if(src.ki >= ki_cost)
				if(!src.doing)
					src.ki_lock = 1
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Kamehameha was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Final Kamehameha, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/Final_Kamehameha_Blast
	icon = 'Final.dmi'