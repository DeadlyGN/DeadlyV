...
mob
	proc
		ArkTwist_Shoot()
			var/obj/tech/H = new/obj/tech/ArkTwist(o=src)
			H.ki_damage = round(src.powerlevel_max * 60)
			var/ki_cost = round(src.ki_max * 4)
			if(src.ki >= ki_cost)
				if(!src.doing)
					src.ki_lock = 1
					...
					sleep(13)
					view(6) << "<font color = white>[src]:</font> Ark Twist!!!"
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Ark Twist slams into you"
								src<<"\red Your Ark Twist slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Ark Twist was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Ark Twist, but it was too strong!"
