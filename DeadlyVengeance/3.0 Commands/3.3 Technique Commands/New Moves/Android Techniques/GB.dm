...
mob
	proc
		Galactic_Buster_Shoot()
			var/obj/tech/H = new/obj/tech/GB(o=src)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Galactic Buster was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Galactic Buster, but it was too strong!"
