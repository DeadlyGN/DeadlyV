...
mob
	proc
		FSKamehameha_Shoot()
			var/obj/tech/H = new/obj/tech/FSKamehameha(o=src)
			H.ki_damage = round(src.powerlevel_max * 410)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Father-Son Kamehawave slams into you"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Father-Son Kamehawave was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Father-Son Kamehawave, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/FSKamehameha
