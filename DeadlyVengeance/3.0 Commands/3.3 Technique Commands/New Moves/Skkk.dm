...
mob
	proc
		Super_Kalo_Ken_Kamehameha_Shoot()
			var/obj/tech/H = new/obj/tech/Super_Kalo_Ken_Kamehameha_Blast(o=src)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Kalo Ken Kamehawave slams into you"
								src<<"\red Your Kalo Ken Kamehawave slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Kalo Ken Kamehawave was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Kalo Ken Kamehawave, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/Super_Kalo_Ken_Kamehameha_Blast
