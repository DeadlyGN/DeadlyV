	...
	verb
		Dark_Renegade(mob/M in oview(6))
			...
			for(var/turf/Planet_Gravitys/King_Kai/D in view(6,M))
				...
				return
			src.afk_time = 0
			var/ki_damage = round(((src.ki_skill / src.ki_skill_max) * src.level) + src.powerlevel_max * 43.721)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					if(prob(7))
						M.powerlevel = 0
						M << "\red [src.name]'s Dark Renegade Blows through your Chest!"
					else
						M.Damage(round(ki_damage))
						M << "\red [src.name]'s Dark Renegade plows into you"
