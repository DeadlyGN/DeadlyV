...
mob
	proc
		Beam_Shoot()
			var/obj/tech/H = new/obj/tech/beam_cannon(o=src)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Special Beam....."
					sleep(13)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Special Beam Cannon slams into you"
								src<<"\red Your Special Beam Cannon slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Special Beam Cannon was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Special Beam Cannon, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/beam_cannon
	icon = 'Beam Cannon.dmi'
	Move()
		var/O = new/obj/trail/BCtrail(src.loc)
		O:icon_state = "trail"
		O:dir = src.dir
		return ..()
obj/trail/BCtrail
