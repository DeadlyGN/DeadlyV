...
mob
	proc
		Light_Grenade_Shoot()
			var/obj/tech/H = new/obj/tech/light_grenade(o=src)
			H.ki_damage = round(src.powerlevel_max * 0.65)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Light Gernade slams into you"
								src<<"\red Your Light Gernade slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Light Gernade was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Light Gernade, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
							del(H)
						sleep(1)
obj/tech/light_grenade
