...
mob
	proc
		Masenko_Shoot()
			var/obj/tech/H = new/obj/tech/masenko(o=src)
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Masenko was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Masenko, but it was too strong!"
...
obj/tech/masenko
	...
	icon_state = "head"
	Move()
		var/O = new/obj/trail/Mtrail(src.loc)
		O:icon_state = "body"
		O:dir = src.dir
		return ..()
obj/trail/Mtrail
