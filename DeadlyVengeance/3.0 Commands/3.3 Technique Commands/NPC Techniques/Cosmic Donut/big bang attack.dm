...
mob
	proc
		Big_Bang_Shoot()
			var/j=input("How much power?") as num
			var/obj/tech/H = new/obj/tech/big_bang(o=src)
			H.ki_damage = j
			if(H.ki_damage < 0)
				src<< "You must input more ki!"
				return
			if(src.powerlevel >= H.ki_damage)
				if(!src.doing)
					src.ki_lock = 1
					spawn(20) src.ki_lock = 0
					if(!src.doing) src.doing ++
					src.Damage(H.ki_damage)
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Big Bang slams into you"
								src<<"\red Your Big Bang slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Big Bang was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Big Bang, but it was too strong!"
