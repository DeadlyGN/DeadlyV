...
mob
	proc
		SKamehameha_Shoot()
			var/obj/tech/H = new/obj/tech/SKamehameha(o=src)
			H.ki_damage = round(src.powerlevel * 0.85)
			var/ki_cost = round(src.ki * 0.65)
			if(src.ki >= ki_cost)
				if(!src.doing)
					src.ki_lock = 1
					...
					while(H)
						...
						for(var/mob/M as mob in T)
							...
							if(!M.dead && !M.safe && M.pk)
								M << "\red [src.name]'s Super Kamehawave slams into you"
								src<<"\red Your Super Kamehawave slams into [M]!"
								...
								if(M.ki_shield)
									...
									if(M.absorbing)
										if(absorb_max > H.ki_damage)
											...
											src<<"\red Your Super Kamehawave was absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											M.ki += H.ki_damage; if(M.ki>M.ki_max*1300){ M.KiExplosion() }
											return
										else M << "\red You tried to absorb [src]'s Super Kamehameha, but it was too strong!"
								M.Damage(H.ki_damage)
								M.BigKiDeathCheck(src)
							del(H)
						sleep(1)
obj/tech/SKamehameha
	icon = 'Kamehameha.dmi'